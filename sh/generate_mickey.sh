    ...
    for prompt in "${prompts[@]}"; do
        for seed in 0 42; do
            ...
            if [ ! -f "$path/casteer.png" ]; then
                python generate_casteer.py --model $model --prompt "$prompt" --num_denoising_steps $num_denoising_steps --seed $seed --output "$path/casteer.png" --steering_vectors mickey_laion_steering_vectors.pickle --alpha 15.0 --steer_type casteer
            fi

            if [ ! -f "$path/mmsteer.png" ]; then
                python generate_casteer.py --model $model --prompt "$prompt" --num_denoising_steps $num_denoising_steps --seed $seed --output "$path/mmsteer.png" --steering_vectors mickey_laion_mm_steering_vectors.pickle --alpha 2.0 --steer_type mmsteer
            fi
        done
    done
done
