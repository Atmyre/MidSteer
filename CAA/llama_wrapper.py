...
class LlamaWrapper:
    def __init__(
        ...
        device: tp.Any = None,
        control_type: tp.Literal['CAA', 'external'] = 'CAA',
    ):
        if device is None:
            device = get_device()
        self.device = device
    ...
    def set_external_controllers(self, external_controllers: list[VectorControl]):
        self._external_controllers = external_controllers

    def set_external_layer_type(self, layer_type: str):
        self._external_layer_type = layer_type

    def set_external_layers_to_steer(self, layers_to_steer: list[int] | None):
        self._external_layers_to_steer = layers_to_steer

    def set_save_internal_decodings(self, value: bool):
        if self._control_type == 'CAA':
            for layer in self.model.model.layers:
                layer.save_internal_decodings = value
        else:
            warnings.warn('set_save_internal_decodings is called with control_type=external')
        ...
        with t.no_grad():
            ...
            if self._control_type == 'CAA':
                ...
                with llm_register_vector_control(
                    model=self.model,
                    control=self._external_controllers,
                    layer_type=self._external_layer_type,
                    layers_to_steer=self._external_layers_to_steer,
                    min_token_index=instr_pos,
                ):
                    generated = self.model.generate(
                        inputs=tokens, max_new_tokens=max_new_tokens, top_k=1
                    )
    ...
    def set_add_activations(self, layer, activations):
        if self._control_type == 'CAA':
            ...
            warnings.warn('set_add_activations is called with control_type=external')

    def set_calc_dot_product_with(self, layer, vector):
        if self._control_type == 'CAA':
            self.model.model.layers[layer].calc_dot_product_with = vector
        else:
            warnings.warn('set_calc_dot_product_with is called with control_type=external')
